...
__global__ void array_mul(int *src1, int *src2, int *dest, int length)
{
    ...
    for (i = block_start; i < block_end; i += blockDim.x)
    {
        if (threadIdx.x + i < block_end)
            dest[threadIdx.x + i] = src1[threadIdx.x + i] * src2[threadIdx.x + i];
    }
}
...
int main()
{
    ...
    int source1[ARRAY_LEN], source2[ARRAY_LEN], destination[ARRAY_LEN];
    int i;
    ...
    cudaMalloc((void **)&d_dest, array_size);
    ...
    array_mul <<<grid_size, block_size>>> (d_src1, d_src2, d_dest, ARRAY_LEN);

    cudaMemcpy(destination, d_dest, array_size, cudaMemcpyDeviceToHost);
